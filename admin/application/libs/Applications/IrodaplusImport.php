...
class IrodaplusImport
{
  ...
  public function importToTermekek()
  {
    ...
    $this->db->query("UPDATE `shop_termekek` as s SET s.xml_import_res_id = (SELECT x.ID FROM xml_temp_products as x WHERE x.origin_id = s.xml_import_origin and s.nagyker_kod = x.prod_id)  WHERE (SELECT x.ID FROM xml_temp_products as x WHERE x.origin_id = s.xml_import_origin and s.nagyker_kod = x.prod_id) != s.xml_import_res_id");
  ...
  {
    $xml = SOURCE . 'irodaplus/ARTICLE_STOCK_EXT.xml';
    $originid = 1;
    ...
    if ( $xml )
    {
      ...
      if ( $parse)
      {
        $telephely = array();
        $tstock = array();
        $insert_stock_row = array();
        $insert_stock_header = array('hashkey', 'telephely_id', 'prod_id', 'me');
        $insert_tstock_row = array();
        $insert_tstock_header = array('hashkey', 'termek_keszlet');

        foreach ( (array)$parse->ARTICLE_STOCKES->children() as $group )
        {
          foreach ((array)$group as $item )
          {
            // Telephely
            $telephely_id = (int)$item->STOCK_ID;
            if ( !array_key_exists($telephely_id, $telephely) ) {
              $telephely[$telephely_id] = (string)$item->STOCK_NAME;
            }

            $hashkey = md5($originid.'_'.$telephely_id.'_'.(int)$item->ARTICLE_ID);
            ...
            // XML készlet collect
            $iid = (int)$item->ARTICLE_ID;
            $tstock[$iid] += (float)$item->QUANTITY;
          }
        }
        unset($parse);
        ...
        if ( $telephely )
        {
          $telephely_insert = array();
          foreach ((array)$telephely as $tid => $telephely ) {
            $thashkey = md5((int)$tid);
            $telephely_insert[] = array(
              $tid,
              $thashkey,
              addslashes($telephely)
            );
          }

          if (isset($telephely_insert))
          {
            $dbx = $this->db->multi_insert(
              'shop_telephely',
              array('ID', 'hashkey', 'telephely'),
              $telephely_insert,
              array(
                'debug' => false,
                'duplicate_keys' => array( 'hashkey', 'telephely' )
              )
            );
            unset($telephely_insert);
          }
          unset($telephely);
        }

        // Készlet stock frissítés
        if (!empty($insert_stock_row))
        {
          $dbx = $this->db->multi_insert(
            'shop_stock',
            $insert_stock_header,
            ...
            array(
              'debug' => false,
              'duplicate_keys' => array( 'hashkey', 'me' )
            )
          );
          unset($insert_stock_row);
        }

        // XML lista stock frissítés
        if ( $tstock )
        {
          foreach ((array)$tstock as $tid => $stock )
          {
            $thashkey = md5($originid.'_'.$tid);
            $insert_tstock_row[] = array(
              $thashkey,
              $stock
            );
          ...
          if (!empty($insert_tstock_row)) {
            $dbx = $this->db->multi_insert(
              'xml_temp_products',
              $insert_tstock_header,
              $insert_tstock_row,
              array(
                'debug' => false,
                'duplicate_keys' => array( 'hashkey', 'termek_keszlet' )
              )
            );
            unset($insert_tstock_row);
          }
          unset($tstock);
        }
  ...
  {
    // MAIN_GROUP
    $xml = SOURCE . 'irodaplus/ARTICLE_MAIN_GROUP.xml';
    ...
    if ( $xml )
    {
      ...
      {
        $insert_row = array();
        $insert_header = array('ID', 'hashkey', 'name', 'description');

        foreach ( (array)$parse->ARTICLE_MAIN_GROUPS->children() as $group )
        {
          foreach ((array)$group as $item ) {
            $hashkey = md5((int)$item->ID);

            $insert_row[] = array(
              (int)$item->ID,
              $hashkey,
              addslashes((string)$item->NAME),
        ...
        if (!empty($insert_row)) {
          $dbx = $this->db->multi_insert(
            ...
            array(
              'debug' => false,
              'duplicate_keys' => array( 'hashkey', 'name','description' )
            )
          );
          unset($insert_row);
        }
    ...
    // SUB_GROUP
    $xml = SOURCE . 'irodaplus/ARTICLE_SUB_GROUP.xml';
    ...
    if ( $xml )
    {
      ...
      if ( $parse)
      {
        ...
        foreach ( (array)$parse->ARTICLE_SUB_GROUPS->children() as $group )
        {
          foreach ((array)$group as $item ) {
            $hashkey = md5((int)$item->ID);
    ...
    // ELEMENT_GROUP
    $xml = SOURCE . 'irodaplus/ARTICLE_ELEMENT_GROUP.xml';
    ...
    if ( $xml )
    {
      ...
      if ( $parse)
      {
        ...
        foreach ( (array)$parse->ARTICLE_ELEMENT_GROUPS->children() as $group )
        {
          foreach ((array)$group as $item ) {
            $hashkey = md5((int)$item->ID);
  ...
  {
    $xml = SOURCE . 'irodaplus/ARTICLE.xml';
    $originid = 1;
    /* */
    if ( $xml ) {
      $parser = new XMLParser( $xml );
      ...
      if ( $parse && $parse->ARTICLES ) {
        ...
        $insert_row = array();
        $insert_header = array('hashkey', 'origin_id', 'MAIN_GROUP_ID', 'SUB_GROUP_ID', 'ELEMENT_GROUP_ID', 'cikkszam', 'prod_id', 'gyarto_kod', 'last_updated', 'termek_nev', 'kisker_ar_netto', 'ar1', 'mennyisegegyseg', 'io');
        $i = 0;
        // Cikkszam
        // CikkszamForm
        // Nev
        ...
        // Afakod
        // Tomeg
        foreach ($parse->ARTICLES->children() as $item ) {
          //$i++;
          //if( $i >= 10) break;
          if((string)$item->ID == '0') continue;
          $hashkey = md5($originid.'_'.(string)$item->ID);

          // Ár fixálás a helytelen formátum végett
          $netto_kisker = $item->NETT_PRICE;
          ...
          $insert_row[] = array(
            $hashkey,
            $originid,
            (int)$item->MAIN_GROUP_ID,
            (int)$item->SUB_GROUP_ID,
            (int)$item->ELEMENT_GROUP_ID,
            addslashes((string)$item->ARTICLE_NUMBER),
            addslashes((int)$item->ID),
            addslashes((string)$item->NAME_B),
            NOW,
            addslashes((string)$item->NAME_A),
            (float)$netto_kisker,
            (float)$item->GROSS_PRICE,
            addslashes((string)$item->UNIT_NAME),
            1 );
        }
        unset($parse);
        ...
        if (!empty($insert_row)) {
          $dbx = $this->db->multi_insert(
            ...
            array(
              'debug' => false,
              'duplicate_keys' => array( 'hashkey','MAIN_GROUP_ID', 'SUB_GROUP_ID', 'ELEMENT_GROUP_ID', 'cikkszam', 'gyarto_kod', 'prod_id', 'last_updated', 'termek_nev', 'kisker_ar_netto', 'ar1', 'mennyisegegyseg', 'io' )
            )
          );
          unset($insert_row);
        }
